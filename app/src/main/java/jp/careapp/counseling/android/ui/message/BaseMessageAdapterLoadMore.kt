...
abstract class BaseMessageAdapterLoadMore<D>(diff: DiffUtil.ItemCallback<D>) :
    ...
    fun checkContainMsg(mailCode: String): Boolean {
        val data = currentList.filter { it is MessageResponse && it.code == mailCode }
        data.forEach {
            if (it is MessageResponse && !it.payFlag) {
                it.open = true
            }
        }
